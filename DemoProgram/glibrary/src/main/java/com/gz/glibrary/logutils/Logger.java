...
import com.gz.glibrary.logutils.file.LogFileParam;
import java.io.File;
...
import static com.gz.glibrary.logutils.LogLevel.TYPE_DEBUG;
import static com.gz.glibrary.logutils.LogLevel.TYPE_ERROR;
import static com.gz.glibrary.logutils.LogLevel.TYPE_INFO;
import static com.gz.glibrary.logutils.LogLevel.TYPE_VERBOSE;
import static com.gz.glibrary.logutils.LogLevel.TYPE_WARM;
import static com.gz.glibrary.logutils.LogLevel.TYPE_WTF;
import static com.gz.glibrary.logutils.utils.ObjectUtil.objectToString;
import static com.gz.glibrary.logutils.utils.Utils.DIVIDER_BOTTOM;
import static com.gz.glibrary.logutils.utils.Utils.DIVIDER_CENTER;
import static com.gz.glibrary.logutils.utils.Utils.DIVIDER_NORMAL;
import static com.gz.glibrary.logutils.utils.Utils.DIVIDER_TOP;
import static com.gz.glibrary.logutils.utils.Utils.largeStringToList;
import static com.gz.glibrary.logutils.utils.Utils.printDividingLine;
...
class Logger implements Printer {
  ...
  private void logString(@LogLevel.LogLevelType int type, String msg, String tag, boolean isPart, Object... args) {
    ...
    if (!isPart) {
      if (args != null && args.length > 0) {
        ...
        } catch (MissingFormatArgumentException | IllegalFormatConversionException | UnknownFormatConversionException e) {
  ...
  private int getStackOffset(StackTraceElement[] trace, Class cla) {
    for (int i = Constant.MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (cla.equals(Logger.class) && i < trace.length - 1 && trace[i + 1].getClassName().equals(Logger.class.getName())) {
        continue;
      }
  ...
  private void writeToFile(String tagName, String logContent, @LogLevel.LogLevelType int logLevel) {
    ...
    if (log2FileConfig.getFileFilter() != null && !log2FileConfig.getFileFilter().accept(logLevel, tagName, logContent)) {
      return;
    }
